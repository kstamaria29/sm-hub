...
serve(async (req) => {
  ...
  let body: WordMasterEndRequest;
  try {
    body = (await req.json()) as WordMasterEndRequest;
  } catch {
  ...
  const { data, error } = await supabase.rpc("word_master_end_game_v1", {
    p_game_id: body.gameId,
    p_actor_user_id: actorUserId,
    p_reason: reason.length > 0 ? reason : "admin_end",
  });
