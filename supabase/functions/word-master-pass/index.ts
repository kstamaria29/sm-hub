...
serve(async (req) => {
  ...
  let body: WordMasterPassRequest;
  try {
    body = (await req.json()) as WordMasterPassRequest;
  } catch {
  ...
  const { data, error } = await supabase.rpc("word_master_pass_turn_v1", {
