-- v1.10: Word Master play-turn scoring upgrades:
-- - strict dictionary validation (ispell when available)
-- - Scrabble-ish scoring: main word + cross words, with letter/word bonuses
...
declare
  ...
  v_turn_points int := 0;
  v_words jsonb := '[]'::jsonb;
  v_invalid_words text[] := array[]::text[];
...
begin
  ...
  -- Extract/score: main word + cross words (bonuses apply only to tiles placed this turn).
  if array_length(v_letters, 1) = 1 then
    v_row := v_rows[1];
    v_col := v_cols[1];
    ...
    if (v_e - v_s + 1) > 1 then
      select
        string_agg(bt.letter, '' order by bt.col),
        sum(bt.points * case when bt.placed_at_turn = v_game.turn_number then bonus.letter_multiplier else 1 end),
        sum(case when bt.placed_at_turn = v_game.turn_number and bonus.word_multiplier = 2 then 1 else 0 end),
        sum(case when bt.placed_at_turn = v_game.turn_number and bonus.word_multiplier = 3 then 1 else 0 end),
        coalesce(array_agg(bonus.label) filter (where bt.placed_at_turn = v_game.turn_number and bonus.label is not null), array[]::text[])
      into v_word, v_base_points, v_dw_count, v_tw_count, v_bonus_labels
      from public.word_master_board_tiles bt
      cross join lateral app.word_master_square_bonus_v1(v_game.board_size, bt.row, bt.col) as bonus
      where bt.game_id = p_game_id
        and bt.row = v_row
        and bt.col between v_s and v_e;

      v_word_multiplier :=
        (power(2::numeric, coalesce(v_dw_count, 0))::int * power(3::numeric, coalesce(v_tw_count, 0))::int);
      v_word_points := coalesce(v_base_points, 0) * coalesce(v_word_multiplier, 1);
      v_turn_points := v_turn_points + v_word_points;
      v_words := v_words || jsonb_build_array(
        jsonb_build_object(
          'direction', 'horizontal',
          'word', v_word,
          'base_points', coalesce(v_base_points, 0),
          'word_multiplier', coalesce(v_word_multiplier, 1),
          'bonuses', to_jsonb(coalesce(v_bonus_labels, array[]::text[])),
          'points', v_word_points
        )
      );

      if not app.word_master_is_valid_word_v1(v_word) then
        v_invalid_words := array_append(v_invalid_words, v_word);
      end if;
    end if;
    ...
    if (v_e - v_s + 1) > 1 then
      select
        string_agg(bt.letter, '' order by bt.row),
        sum(bt.points * case when bt.placed_at_turn = v_game.turn_number then bonus.letter_multiplier else 1 end),
        sum(case when bt.placed_at_turn = v_game.turn_number and bonus.word_multiplier = 2 then 1 else 0 end),
        sum(case when bt.placed_at_turn = v_game.turn_number and bonus.word_multiplier = 3 then 1 else 0 end),
        coalesce(array_agg(bonus.label) filter (where bt.placed_at_turn = v_game.turn_number and bonus.label is not null), array[]::text[])
    ...
    if v_orientation = 'horizontal' then
      ...
      -- Vertical cross words (one per newly placed tile, if any)
      ...
      -- Horizontal cross words (one per newly placed tile, if any)
      for v_i in 1..array_length(v_rows, 1) loop
        v_row := v_rows[v_i];
        v_col := v_cols[v_i];
  ...
  if coalesce(array_length(v_invalid_words, 1), 0) > 0 then
    select array_agg(distinct w order by w)
    into v_invalid_words
    from unnest(v_invalid_words) as t(w);

    raise exception 'invalid word(s): %', array_to_string(v_invalid_words, ', ') using errcode = '23514';
  end if;

  if array_length(v_letters, 1) = v_game.rack_size then
    v_bingo_bonus := 50;
    v_turn_points := v_turn_points + v_bingo_bonus;
  end if;
